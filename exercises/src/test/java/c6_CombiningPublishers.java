...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void task_executor_again() {
        Flux<Void> tasks = taskExecutor()
                // concatMap doesn't allow inner Streams to interleave, while
                // flatMap allows. concatMap waits for one inner Publisher to complete
                // before generating the next one and subscribing to it. flatMapSequential
                // eagerly subscribes to all inner Publishers, while queueing elements
                // from late inners until all elements from earlier inners have been emitted.
                .concatMap(Mono::then);
    ...
    /**
     * You are writing software for broker house. You can retrieve current stock prices
     * by calling either
     * `getStocksGrpc()` or `getStocksRest()`.
     * Since goal is the best response time, invoke both services but use result
     * only from the one that responds first.
     */
    @Test
    public void need_for_speed() {
        Flux<String> stonks = Flux.firstWithValue(List.of(getStocksGrpc(), getStocksRest()));
    ...
    /**
     * As part of your job as software engineer for broker house, you have also
     * introduced quick local cache to retrieve
     * stocks from. But cache may not be formed yet or is empty. If cache is empty,
    ...
    /**
     * You are checking mail in your mailboxes. Check first mailbox, and if first
     * message contains spam immediately
     ...
     * When user types in a text box results should appear in near real-time with each
     * keystroke.
     * <p>
     * Call `autoComplete()` function for each user input
     * but if newer input arrives, cancel previous `autoComplete()` call and call it
     * for latest input.
     */
    @Test
     ...
     * Orchestrate file writing operations in a self-explanatory way using operators
     * like `when`,`and`,`then`...
    ...
    /**
     * Execute all tasks sequentially and after each task have been executed,
     * commit task changes. Don't lose id's of
    ...
    public void acid_durability() {
        Flux<String> committedTasksIds = tasksToExecute()
                .concatMap(t -> t.flatMap(id -> commitTask(id).thenReturn(id)));
     ...
     * Merge two companies, so they may still produce titles in individual pace but as a
     * single company.
     */
    @Test
    ...
    /**
     * Your job is to produce cars. To produce car you need chassis and engine that
     * are produced by a different
     * manufacturer. You need both parts before you can produce a car.
     * Also, engine factory is located further away and engines are more complicated
     * to produce, therefore it will be
