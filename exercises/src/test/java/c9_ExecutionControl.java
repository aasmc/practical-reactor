...
/**
 * With multi-core architectures being a commodity nowadays, being able to easily parallelize
 * work is important.
...
public class c9_ExecutionControl extends ExecutionControlBase {
     ...
     * multiple notifications, for better UX you want to slow down appearance between notifications
     * by 1 second.
     * Pay attention to threading, compare what code prints out before and after solution.
     * Explain why?
     */
    @Test
    ...
    /**
     * You are using free access to remote hosting machine. You want to execute 3 tasks
     * on this machine, but machine
     * will allow you to execute one task at a time on a given schedule which is orchestrated
     * by the semaphore. If you
    ...
    public void free_runners() {
        ...
        Flux<Void> taskQueue = Flux
                .just(task, task, task)
                .flatMap(m -> m.subscribeOn(Schedulers.boundedElastic()), 3);
        //don't change code below
    ...
    public void sequential_free_runners() {
        Flux<String> tasks = tasks()
                .flatMapSequential(t -> t.subscribeOn(Schedulers.boundedElastic()), 3);
     ...
     * - filtering events that have metadata, printing out metadata, and mapping to
     * json can be done in parallel.
     * Then branch in before appending events to store. `appendToStore` must be invoked
     * sequentially!
     */
    @Test
