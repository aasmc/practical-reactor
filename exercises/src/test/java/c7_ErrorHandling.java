...
public class c7_ErrorHandling extends ErrorHandlingBase {
     ...
     * Ignore any failures, and if error happens finish consuming silently without
    ...
    /**
     * `getFilesContent()` should return files content from three different files.
     * But one of the files may be
    ...
    public void resilience() {
        Flux<String> content = getFilesContent()
                .flatMap(f -> {
                    return f.onErrorResume(e -> Mono.empty());
                });
    ...
    /**
     * You are trying to read temperature from your recently installed DIY IoT
     * temperature sensor. Unfortunately, sensor
     * is cheaply made and may not return value on each read. Keep retrying
    ...
    /**
     * In following example you are trying to establish connection to database,
     * which is very expensive operation.
     ...
     * Implement polling
     * mechanism by invoking `nodeAlerts()` repeatedly until you get all (2) alerts.
     * If you get empty result, delay next
