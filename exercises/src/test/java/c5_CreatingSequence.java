...
public class c5_CreatingSequence {
    ...
    /**
     * Create Callable that increments the counter and returns the counter value,
     * and then use `repeat()` operator to create Flux that emits
     * values from 0 to 10.
     */
    @Test
    public void repeat() {
        ...
        Flux<Integer> repeated = mono.repeat(9);
     ...
     * We will learn how to use them in a
    ...
    public void generate_programmatically() {
        BiFunction<Integer, SynchronousSink<Integer>, Integer> generator = (state, sink) -> {
            sink.next(state);
            if (state == 5) {
                sink.complete();
            }
        ...
        Callable<Integer> initial = () -> 0;
        Flux<Integer> generateFlux = Flux.generate(initial, generator);
