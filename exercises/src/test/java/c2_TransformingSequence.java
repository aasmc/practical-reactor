...
public class c2_TransformingSequence extends TransformingSequenceBase {
    ...
    public void transforming_sequence_2() {
        ...
        Flux<String> resultSequence = numbersFlux.handle((i, sink) -> {
            if (i > 0) {
                sink.next(">");
            } else if (i == 0) {
                sink.next("=");
    ...
    /**
     * `object_service()` streams sequence of Objects, but if you peek into service
    ...
    /**
     * Reduce the values from `numerical_service()` into a single number that is
     * equal to sum of all numbers emitted by
     * this service.
     */
    @Test
